...
class JointEmbedding(nn.Module):
    ...
    def attention_position(self, dim, input_tensor):
        ...
        i = torch.arange(dim, dtype=torch.long).to(device)
        t = (2 * i / dim)
...
class AttentionHead(nn.Module):
    ...
    def forward(self, input_tensor: torch.Tensor, attention_mask: torch.Tensor = None):
        ...
        d = query.size(1) ** 0.5
        scores = torch.bmm(query, key.transpose(1, 2)) / d

        scores = scores.masked_fill_(attention_mask, -1e9) # puts -inf in the padding indices
...
class Stack_Encoder(nn.Module):
    def __init__(self, dim_inp, dim_out, attention_heads=4, dropout=0.1, num_encoders = 6):
        super().__init__()
        self.stacked_encoders = nn.ModuleList([
            Encoder(dim_inp, dim_out, attention_heads, dropout) for _ in range(num_encoders)
        ])
    
    def forward(self, input_tensor: torch.Tensor, attention_mask: torch.Tensor = None):
        x = self.stacked_encoders[0](input_tensor, attention_mask)
        # x = self.stacked_encoders[1](x)

        for enc in self.stacked_encoders[1:]:
...
class BERT(nn.Module):
    ...
    def __init__(self, vocab_size, dim_inp, dim_out, attention_heads=4):
        ...
        self.encoder = Stack_Encoder(dim_inp, dim_out, attention_heads)
    ...
    def forward(self, input_tensor: torch.Tensor, attention_mask: torch.Tensor):
        ...
        first_word = encoded[:, 0, :]  #first word is the classification token '[CLS]'
