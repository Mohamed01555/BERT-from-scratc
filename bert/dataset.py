...
class IMDBBertDataset(Dataset):
    ...
    def _find_optimal_sentence_length(self, lengths: typing.List[int]):
        '''
        Instead of hard coeded max length of sentences we use _find_optimal_sentence_length
        to find it statistically depending on 70% of sentence lengths.
        '''
    ...
    def _pad_sentence(self, sentence: typing.List[str], inverse_token_mask: typing.List[bool] = None):
        ...
        if len_s >= self.optimal_sentence_length:
            padded_s = sentence[:self.optimal_sentence_length]
        else:
            padded_s = sentence + [self.PAD] * (self.optimal_sentence_length - len_s)
        ...
        if inverse_token_mask:
            ...
            if len_m >= self.optimal_sentence_length:
                padded_inverse_token_mask = inverse_token_mask[:self.optimal_sentence_length]
            else:
                padded_inverse_token_mask = inverse_token_mask + [True] * (self.optimal_sentence_length - len_m)
        return padded_s, padded_inverse_token_mask 
    ...
    def _preprocess_sentence(self, sentence: typing.List[str], should_mask: bool = True):
        ...
        padded_sentence, padded_inverse_token_mask = self._pad_sentence([self.CLS] + sentence, [True] + inverse_token_mask)

        return padded_sentence, padded_inverse_token_mask
    ...
    def prepare_dataset(self) -> pd.DataFrame:
        ...
        2-Create vocabulary for word - token pair, for example {'go': 45}
    ...
    def __getitem__(self, idx):
        ...
        inp = torch.Tensor(item[self.MASKED_INDICES_COLUMN]).long() #review words indices
        token_mask = torch.Tensor(item[self.TOKEN_MASK_COLUMN]).bool() #review after masking some words, all entries are true except masked words

        mask_target = torch.Tensor(item[self.TARGET_COLUMN]).long() #indexes of masked words , all entries are zero except masked words
        mask_target = mask_target.masked_fill_(token_mask, 0)

        attention_mask = (inp == self.vocab[self.PAD]).unsqueeze(0) #boolean tensor ,all entries are False except padding token 
...
if __name__ == '__main__':
    ...
    ds = IMDBBertDataset(BASE_DIR.joinpath('data/imdb.csv'), ds_from=0, ds_to=50,
                         should_include_text=True)
    # print(ds.df)
    print(ds[0],ds.df.iloc[0], '\n\n',ds.__getitem__(0) )
